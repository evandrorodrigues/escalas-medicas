...
export default function App() {
  ...
  return (
    <div className="flex flex-col items-center p-6">
      ...
      <input type="text" placeholder="Nome do Médico" value={medico} onChange={(e) => setMedico(e.target.value)} className="border p-2 w-full mb-2"/>
      <input type="text" placeholder="CNPJ do Médico" value={cnpj} onChange={(e) => setCnpj(e.target.value)} className="border p-2 w-full mb-2"/>
      <input type="text" placeholder="Coordenação" value={coordenacao} onChange={(e) => setCoordenacao(e.target.value)} className="border p-2 w-full mb-2"/>
      
      <Select options={[{ label: "Disponibilidade", value: "Disponibilidade" }, { label: "Alteração", value: "Alteração" }, { label: "Cancelamento", value: "Cancelamento" }]}
      ...
      {unidades.map((unidade, uIndex) => (
        <div key={uIndex} className="mb-4 p-4 border w-full">
          <div className="flex justify-between items-center">
            <Select options={unidadesLista.map((u) => ({ label: u, value: u }))}
          ...
          {unidade.dias.map((dia, dIndex) => (
            <div key={dIndex} className="flex gap-2 my-2">
              <input type="date" min={new Date().toISOString().split("T")[0]}
                value={dia.data} onChange={(e) => updateData(uIndex, dIndex, "data", e.target.value)}
                className="border p-2"/>
              <Select options={horariosLista.map((h) => ({ label: h, value: h }))}
